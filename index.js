...
document
  ...
  .addEventListener(touchEvent, async () => {
...
const getRandomIndex = items =>
  Math.floor(Math.random() * Math.floor(items.length - 1));
...
const generateEnemies = () => {
  ...
  if (isSafari()) {
    ...
    enemyAnimation = enemy.animate(
      {
        transform: [
          `translate(-${window.innerWidth / 2 + 50}px, 0)`,
          `translate(${window.innerWidth + 50}px, 0)`
        ]
      },
      {
        duration: 8000
...
const collisionWithEnemy = () => {
  console.log(`emoji vale: ${emoji}`);
  console.log(`enemy vale: ${enemy}`);
  ...
  const collisionFromRight =
    emojiLeft < enemyRight && emojiLeft > enemyLeft ? true : false;
  const collisionFromLeft =
    emojiRight > enemyLeft && emojiRight < enemyRight ? true : false;
